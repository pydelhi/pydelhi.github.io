function display_next_event(el) {
    ...
    var caption = captions[Math.floor(Math.random()*captions.length)],
        meetup_url = $(el).attr('data-meetup-url');
    $.ajax({
      url: 'https://api.meetup.com/'+meetup_url+'?photo-host=public&sig_id=44948372&only=next_event&sig=03b1cf02afd70a3ae78cc9c8cc83d514d6f37ecf',
      jsonp: 'callback',
      dataType: 'jsonp',
      success: function( response ) {
          ...
          if (next_event){
            var html = '<span class=caption>'+ caption +'</span><a target="_blank" href=http://www.meetup.com/pydelhi/events/' + next_event.id + ' >' + next_event.name + '</a>';
